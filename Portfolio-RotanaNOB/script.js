...
function setIcon(btn, name){
  if(!btn) return;
  btn.innerHTML = `<i data-feather="${name}"></i>`;
  try{ if(window.feather) feather.replace(); }catch(e){}
}

function applyDark(isDark){
  document.body.classList.toggle('dark-mode', isDark);
  setIcon(darkToggle, isDark ? 'sun' : 'moon');
}
function initTheme(){
...
setIcon(darkToggle, document.body.classList.contains('dark-mode') ? 'sun' : 'moon');
// initial weather & sound icons
setIcon($('#weather-toggle'), 'cloud-rain');
// note: `soundToggle` is declared later — initialize its icon after it's defined
...
// now that `soundToggle` exists, set its initial icon
...
if(canvas && ctx){
  ...
  function setWeather(next){
    weatherMode = next;
    const icon = next==='both' ? 'cloud-rain' : next==='rain' ? 'cloud-rain' : 'cloud';
    setIcon(weatherToggle, icon);
...
const demoViewer = $('.demo-viewer');
const demoThumbs = $('.demo-thumbs');

function closeDemo(){
  if(!demoModal) return;
  demoModal.setAttribute('aria-hidden','true');
  demoViewer.innerHTML = '';
  demoThumbs.innerHTML = '';
}

function openDemo(list){
  if(!demoModal) return;
  demoViewer.innerHTML = '';
  demoThumbs.innerHTML = '';
  const items = list.map(s => decodeURIComponent(s.trim()));
  // show first item
  const showIndex = (i)=>{
    demoViewer.innerHTML = '';
    const src = items[i];
    if(src.match(/\.mp4$|\.webm$/i)){
      const v = document.createElement('video'); v.controls = true; v.autoplay = true; v.playsInline = true; v.src = src; demoViewer.appendChild(v);
    } else {
      const img = document.createElement('img'); img.src = src; img.alt = 'Demo image'; demoViewer.appendChild(img);
    }
  };
  items.forEach((it, idx)=>{
    const b = document.createElement('button');
    b.type = 'button';
    if(it.match(/\.mp4$|\.webm$/i)){
      const th = document.createElement('video'); th.src = it; th.preload = 'metadata'; th.width = 120; th.height = 68; th.muted = true; th.style.objectFit='cover'; b.appendChild(th);
    } else {
      const th = document.createElement('img'); th.src = it; th.width = 120; th.height = 68; th.style.objectFit='cover'; b.appendChild(th);
    }
    b.addEventListener('click', ()=> showIndex(idx));
    demoThumbs.appendChild(b);
  });
  showIndex(0);
...
$$('.project-actions button').forEach(btn=>{
  btn.addEventListener('click', ()=>{
    const data = btn.getAttribute('data-gallery') || '';
    if(!data) return alert('No demo media configured');
...
// close handlers
demoModal?.addEventListener('click', (e)=>{ if(e.target.matches('[data-close], .demo-backdrop')) closeDemo(); });
